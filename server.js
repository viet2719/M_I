...
app.prepare().then(() => {
  ...
  server.get("/(:title)-p(:id).html", (req, res) => {
    const { id } = req.params;
    return app.render(req, res, `/chi_tiet_nganh_nghe`, { id: parseInt(id), ...req.query });
  });
  // Tìm kiếm theo tỉnh thành
  server.get("/tim-viec-(:title)-c(:id)", (req, res) => {
    const { id } = req.params;
    return app.render(req, res, `/tim_kiem_tinh_thanh`, { id: parseInt(id), ...req.query });
  });
